...
def time_current():
    ...
    return current_time
##obtain options
parser = argparse.ArgumentParser(description='''This scripts assign species level taxa based on the gtdb-tk results and local dRep cluster, 
                                and find out MAGs which are uncharacterized or brand-new species. The dRep software is required.''', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
taxa_change_log_df = pd.DataFrame(columns=['Original_taxonomy (gtdb-tk)', 'New_taxonomy', 'Original_dRep_id', 'Final_species_ids', 'Lineage_source', 'Modified_lineage', 'Uncharacterized_species (Yes/No)', 'New_species (Yes/No)', 'NOTE'], index=all_genomes, data='')
